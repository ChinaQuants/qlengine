...
void DayCounterTest::testActualActualNoLeap() {
    ...
    Time expected[] = {0.29863013698630, 0.16574585635359};

    for (Size i = 0; i < testStartDates.size(); ++i) {
        Time calculated =
            dayCounter.yearFraction(testStartDates[i], testEndDates[i], testRefStartDates[i], testRefEndDates[i]);
        if (std::fabs(calculated - expected[i]) > 1.0e-12) {
            BOOST_ERROR("from " << testStartDates[i] << " to " << testEndDates[i] << ":\n"
